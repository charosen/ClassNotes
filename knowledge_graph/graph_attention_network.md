...
通过diff比较kipf&welling数据集和zhilin yang数据，发现transductive部分是完全一样的（kipf&welling就是沿用的zhilin yang的数据集）
...
沿用论文`Inductive representation learning on large graphs`（就是graphsage）的预处理数据集；

附上论文的github链接，可以观察数据集：

1. <https://github.com/williamleif/GraphSAGE>
2. github库中还提供了一个小版本的example_data，可以用来观察与了解ppi数据集的格式；
3. ![-w1021](media/15637813645333.jpg)


ppi原始数据集

1. 分析：
    1. G.json: networkx specified json文件，以node，link等键描述图的节点特性、连接关系，图中节点哪些为训练/验证/测试集数据
    2. id_map.json: 图节点id映射为feature矩阵中的行索引
    3. class_map.json: 图节点id映射为类标签
    4. feats.npy: 节点特征的矩阵
    5. walks.txt: 应该用不上
    6. ![](media/15637793883964.jpg)
2. 特征
    1. ![](media/15637798370316.jpg)
...
1. cora:
    1. sparse --avg acc 0.83333325: [0.8369999527931213, 0.8220000267028809, 0.841999888420105, 0.8410000205039978, 0.8329999446868896, 0.8309999704360962, 0.8369999527931213, 0.8349999785423279, 0.8299999833106995, 0.8279999494552612, 0.8359999060630798, 0.8240000605583191, 0.8299999833106995, 0.8329999446868896, 0.8369999527931213, 0.8349999785423279, 0.8369999527931213, 0.8339999914169312, 0.8319999575614929, 0.8309999704360962, 0.8199999332427979, 0.8349999785423279, 0.8349999785423279, 0.8249999284744263, 0.8369998931884766, 0.8399999141693115, 0.8339999914169312, 0.8299999833106995, 0.8379999995231628, 0.8410000205039978]
    2. non-sparse --avg acc 0.83113325:[0.8389999866485596, 0.8420000076293945, 0.8339999914169312, 0.8309999704360962, 0.8279999494552612, 0.8359999656677246, 0.828999936580658, 0.8329999446868896, 0.8359999656677246, 0.8229999542236328, 0.8329999446868896, 0.8379998803138733, 0.8339999914169312, 0.8309999704360962, 0.8349999785423279, 0.8299999833106995, 0.8290000557899475, 0.8319999575614929, 0.8159999251365662, 0.8329999446868896, 0.8339999914169312, 0.8359999656677246, 0.8410000205039978, 0.8319999575614929, 0.8239999413490295, 0.8339999914169312, 0.8140000104904175, 0.8220000267028809, 0.8269999623298645, 0.8280000686645508]
2. citeseer:
    1. sparse --avg acc 0.72613335: [0.7229999899864197, 0.7299999594688416, 0.7299999594688416, 0.7149999737739563, 0.7279999852180481, 0.7299999594688416, 0.7249999642372131, 0.7329999804496765, 0.7260000109672546, 0.7259999513626099, 0.7359999418258667, 0.734000027179718, 0.7359999418258667, 0.7240000367164612, 0.7280000448226929, 0.7329999804496765, 0.7350000143051147, 0.7269999980926514, 0.7380000352859497, 0.7269999980926514, 0.7279999852180481, 0.7199999690055847, 0.7239999175071716, 0.7080000042915344, 0.7240000367164612, 0.718999981880188, 0.7249999642372131, 0.7089999318122864, 0.7330000400543213, 0.7099999785423279]
    2. non-sparse -- avg acc 0.72193336: [0.7289999723434448, 0.7269999980926514, 0.722000002861023, 0.7170000076293945, 0.7240000367164612, 0.7170000076293945, 0.7240000367164612, 0.7160000205039978, 0.7160000801086426, 0.7139999866485596, 0.718999981880188, 0.7179999947547913, 0.7120000123977661, 0.7329999804496765, 0.7229999899864197, 0.7350000739097595, 0.7330000400543213, 0.7290000319480896, 0.7199999690055847, 0.7269999980926514, 0.7089999914169312, 0.7269999980926514, 0.7210000157356262, 0.7110000252723694, 0.7250000834465027, 0.722000002861023, 0.7290000319480896, 0.7249999046325684, 0.7280000448226929, 0.706000030040741]
3. pubmed:
    1. sparse: 0.7669, 0.7770, 0.7709, 0.7809, 0.7779
    2. non-sparse: 

观察源码发现，sparse版本与non-sparse版本不太一样，分析如下

non-sparse版本在预处理时将邻接矩阵为0的元素，即不连接的关系置成负无穷，为1的元素置0，然后将处理后的偏置邻接矩阵bias加在leakyrelu输出上，经过softmax可以完全屏蔽不相邻元素的影响，因为softmax中的指数函数可以将负无穷置0，如下图所示。

![](media/15637633581117.jpg)


但是，sparse版本则不同，它是直接将邻接矩阵adj_mat乘以leakyrelu的输入，将对应元素置0，经过leakyrelu后仍为0，则在softmax后对应项为一个很小的但不为0的注意力，所以并未完全屏蔽不相连元素的影响，如下图所示
![](media/15637628751309.jpg)

对于inductive ppi数据集，实验效果为92.5%；对于transductive pubmed数据集，实验效果为78%，均比论文中提及的低，仍待进一步修改实验：

![](media/15638899733135.jpg)
