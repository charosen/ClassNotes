...
1. 维度的灾难：当数据的维度增加后，数据在其维度空间中就变得稀疏了（想象一下高维空间和点即可知）；数据之间的距离就变小了；
...
3. 属性类型：
    ...
    2. 二元属性binary：只有0或1两种状态的标称属性；
        ...
        2. 非对称二元属性：两种状态重要程度不同，对更重要的状态用1编码，更关心1状态；
        3. 平均数、中位数无意义，众数有意义
...
1. 数据矩阵：两种模式，行表示数据对象，列表示数据属性，~~或者，行表示数据属性，列表示数据对象~~；
2. 相异性矩阵：一种模式，行列都是数据对象，元素为数据对象的距离；对称矩阵，也可用下三角矩阵表示；
